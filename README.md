...
- Binary columns: a `Vec<u8>` (or `Option<Vec<u8>>`) field is mapped to a `bytea` column and is passed to postgres as a binary parameter. Put `#[sql_type("json")]` or `#[sql_type("jsonb")]` on it to serialize it as a json array of numbers instead. Other `Vec<T>` fields still require an explicit `#[sql_type("json")]`/`#[sql_type("jsonb")]`. A `Vec<u8>` used as a where field is a compile time error, since `Vec<T>` in a where model renders an `IN (...)` list which would not match a binary value - use `Vec<i16>` when an `IN (...)` list of numbers is meant.
